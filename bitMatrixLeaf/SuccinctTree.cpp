...
SuccinctTree SuccinctTree::Union(const SuccinctTree& A, size_t nodeA,
    ...
    SuccinctTree result(dim); result.giveChild();
		...
		// check for base case!!!
		// do a whole separate thing otherwise I'm gunna be confused as fuck
		if (dim == LEAF_MUM) {
			for (size_t i = 0; i < NUM_CHILDREN; ++i) {
				result.set(HEAD_NODE, i, A.get(nodeA, i) | B.get(nodeB, i));		
			}
			return result;
		}
			...
			for (size_t i = 0; i < ROW_SIZE; ++i) {
				for (size_t j = 0; j < ROW_SIZE; ++j) {
					...
					for (size_t k = 0; k < ROW_SIZE; ++k) {
						entry |= mult(A.get(nodeA, i, k), B.get(nodeB, k, j));
					}
...
size_t SuccinctTree::insert(std::string child) {
  ...
  for (size_t i = 0; i < NUM_CHILDREN; ++i) {
    ...
    // check for base case here, when grandChild.length()
		if (grandChild.length() == LEAF_SIZE) {
			//then do some shit
			uint64_t entry = std::stoull(grandChild, 0, 2);
			ruler[index + i] = entry;
		} else {
			// normal recursive case shit
